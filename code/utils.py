...
def readCsv(csvfname,add=False,cols=False):
    # read csv to list of lists
    with open(csvfname, 'r') as csvf:
        reader = csv.reader(csvf)
        csvlines = list(reader)
    if add != False:
        chars = pd.read_csv(add, names = cols)
        charslist = chars.values.tolist()
    for it in range(len(csvlines)):
        csvlines[it] = csvlines[it] + charslist[it]
    return csvlines
