...
def calcFleischner(nodules):
    ...
    for lndU in np.unique(LND):
        ...
        #fleischner.append([int(lndU),f,p0,p1,p2,p3])
        fleischner.append([int(lndU),f])
    fleischner = np.array(fleischner)
#        f_return = np.array([len(f),2])
#        f_return[:,0] = int(lndU)
#        f_return[:,1] = f
    return fleischner
...
#    
    fname_gtNodulesFleischner = 'predictedNodules.csv'
    gtNodules = readCsv(fname_gtNodulesFleischner)
    gtNodules = joinNodules(gtNodules)
    pdFleischner1 = calcFleischner(gtNodules)
    
    fname_gtNodulesFleischner = 'predictedNodules2.csv'
    gtNodules = readCsv(fname_gtNodulesFleischner)
    gtNodules = joinNodules(gtNodules)
    pdFleischner2 = calcFleischner(gtNodules)    
